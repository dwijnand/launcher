...
object Pre {
  ...
  def trimLeading(line: String): String = {
    @tailrec def newStart(i: Int): Int =
      if (i >= line.length || !Character.isWhitespace(line.charAt(i))) i else newStart(i + 1)
    line.substring(newStart(0))
  }

  def isEmpty(line: String): Boolean = line.length == 0
  def isNonEmpty(line: String): Boolean = line.length > 0
  ...
  def toBoolean(s: String): Boolean = java.lang.Boolean.parseBoolean(s)

  def toArray[T: ClassTag](list: List[T]): Array[T] = {
    val arr = new Array[T](list.length)
    @tailrec def copy(i: Int, rem: List[T]): Unit =
      if (i < arr.length) {
        arr(i) = rem.head
        copy(i + 1, rem.tail)
      }
  ...
  def orElse[T](opt: Option[T], default: T): T = if (opt.isDefined) opt.get else default
  ...
  def getMissing(loader: ClassLoader, classes: Iterable[String]): Iterable[String] = {
    def classMissing(c: String) = try { Class.forName(c, false, loader); false } catch {
  ...
  private[boot] def readProperties(propertiesFile: File) = {
    ...
    if (propertiesFile.exists) Using(new FileInputStream(propertiesFile))(properties.load)
    properties
  }
