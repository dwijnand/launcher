...
object Configuration {
  ...
  def parse(file: URL, baseDirectory: File): LaunchConfiguration =
    Using(new InputStreamReader(file.openStream, "utf8"))((new ConfigurationParser).apply)

  /** Finds the configuration location. Note: Configuration may be previously serialized by a launcher. */
  @tailrec
  def find(args: List[String], baseDirectory: File): (URL, List[String], ConfigurationStorageState.Value) =
    args match {
  ...
  def configurationFromFile(path: String, baseDirectory: File): URL = {
    ...
    def resolve(against: URI): Option[URL] = {
      ...
      val exists = try new File(resolved).exists
    ...
    if (!resolving.hasNext)
      multiPartError(s"Could not find configuration file '$path'.  Searched:", against)
    resolving.next()
  }

  def multiPartError[T](firstLine: String, lines: List[T]): Nothing =
    Pre.error((firstLine :: lines).mkString("\n\t"))

  def UnspecifiedVersionPart: String = "Unspecified"
  def DefaultVersionPart: String = "Default"
  def DefaultBuildProperties: String = "project/build.properties"
  def SbtVersionProperty: String = "sbt.version"
  val ConfigurationName: String = "sbt.boot.properties"
  val JarBasePath: String = "/sbt/"
  def userConfigurationPath: String = s"/$ConfigurationName"
  def defaultConfigurationPath: String = JarBasePath + ConfigurationName
  ...
  def resourcePaths(sbtVersion: Option[String]): List[String] =
    for (part <- versionParts(sbtVersion); base <- baseResourcePaths) yield base + part

  def fallbackParts: List[String] = "" :: Nil

  def versionParts(version: Option[String]): List[String] = version match {
  ...
  def versionParts(version: String): List[String] = {
    ...
    if (m.matches()) subPartsIndices flatMap (is => fullMatchOnly(is.map(m.group)))
    else noMatchParts
  }
  ...
  def filePathURI(path: String): URI =
    ...
    else new URI(if (new File(path).isAbsolute) "file" else null, path, null)
  ...
  def directoryURI(uri: URI): URI = {
    ...
    val dirStr = if (str.endsWith("/")) str else s"$str/"
